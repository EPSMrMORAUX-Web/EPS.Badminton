...
### 💾 Sauvegarde complète (en haut de l'onglet)

| Bouton | Action |
|---|---|
| **⬇️ Exporter la sauvegarde** | Télécharge un fichier `.json` contenant **toutes** les données : élèves, niveaux, poules + résultats, feuilles de match, zones dangereuses, scores ATP, barèmes BAC |
| **⬆️ Importer une sauvegarde** | Restaure l'intégralité d'un fichier `.json` exporté précédemment |

> 🔑 **Usage clé** : avant de fermer l'application, de changer d'appareil ou de vider les données → **Exporter la sauvegarde**. Au prochain cours → **Importer une sauvegarde**. Vous retrouvez exactement là où vous en étiez : poules constituées, scores, tout.

Le fichier est nommé automatiquement : `BadmintonPro_sauvegarde_JJ-MM-AAAA_Xeleves.json`

### Export partiel
| Bouton | Contenu | Usage |
|---|---|---|
| **⬇️ Export Excel + Niveaux** | Nom, Prénom, Niveau (`.xlsx`) | Sauvegarde du diagnostic uniquement |
| **⬇️ Export CSV simple** | Nom, Prénom, Niveau (`.csv`) | Usage bureautique |
...
> ⚠️ **Important** : le `localStorage` est lié au navigateur et à l'origine du fichier. Si vous ouvrez le fichier depuis un autre appareil ou un autre navigateur, les données ne seront pas présentes. Utilisez la **sauvegarde complète** (`.json`) pour transférer toutes vos données entre sessions ou appareils.
...
3. (1ère fois) Onglet Élèves → Importer le fichier classe Excel/CSV
4. (séances suivantes) Cliquer "⬆️ Importer une sauvegarde" → sélectionner
   le fichier .json → tout est restauré : élèves, poules, scores, barèmes
...
8. Saisir les scores en temps réel (tablette — drag tactile natif)
...
12. Onglet Élèves → "⬇️ Exporter la sauvegarde" → garder ce .json précieusement
    (il contient TOUT : élèves, poules, matchs, scores, barèmes)
```
